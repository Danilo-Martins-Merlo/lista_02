...
Função multiplicacaoDeMatrizes(matrizA, matrizB):
    ...
    Se tamanho(matrizA[0]) != tamanho(matrizB) então:
        Imprimir ("A quantidade de linhas da matriz A tem que ser igual a quantidade de colunas da matriz B")
    Senão:
        var linhasA <- tamanho(matrizA)
        var colunasA <- tamanho(matrizA[0])
        var colunasB <- tamanho(matrizB[0])
...
matrizResultante <- MultiplicacaoDeMatrizes(matrizA, matrizB)
Imprimir("Multiplicação das matrizes:")
